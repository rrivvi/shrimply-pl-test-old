...
pub struct KeyframeGraphDraw<'a> {
    ...
    pub frame_step: Time,
    pub scrollbar: Option<shrimply_skia_adw_ui::Scrollbar>,
    pub overscroll: Option<(shrimply_skia_adw_ui::Edge, f64)>,
    pub playhead: Time,
...
fn draw_bool_keys(frame: GraphFrame<'_>, points: &[KeyframePoint]) {
    let graph = shrimply_discrete_keyframe_graph_ui::Graph::new(
        points.iter().map(|point| point.time).collect(),
    );
    shrimply_discrete_keyframe_graph_ui::draw(shrimply_discrete_keyframe_graph_ui::Draw {
...
fn draw_graph_overscroll(
    ...
    width: f64,
    content_height: f64,
    edge: shrimply_skia_adw_ui::Edge,
    distance: f64,
) {
