...
use clip_items::*;
pub(super) use clip_items::{item_rect, row_screen_y, row_y};
use folded::{draw_expanded_audio_tracks, draw_expanded_video_tracks};
use icons::*;
pub(super) use previews::active_virtual_tracks;
use previews::*;
use track_controls::{draw_track_label_pane, track_kind_label_color};
use tracks::draw_tracks;

pub(super) struct TimelineInput<'a, 'b> {
    pub(super) painter: &'a TimelinePainter,
    pub(super) project: &'a Project,
    pub(super) playback_performance: &'a playback_performance::Snapshot,
    pub(super) current_time: Time,
    pub(super) waveforms: &'a WaveformMap,
    pub(super) beats: &'a BeatMap,
    pub(super) beat_grid_enabled: bool,
    pub(super) selected_items: &'a [ItemKey],
    pub(super) selected_nested_items: &'a [crate::project::ItemAddress],
    pub(super) selected_tracks: &'a [crate::project::TrackAddress],
    pub(super) selected_gap: Option<TrackGap>,
    pub(super) track_control_draw: &'a mut TrackControlDraw<'b>,
    pub(super) dragged_group: Option<&'a DraggedGroup>,
    pub(super) folded_drag: Option<&'a folded_sequence::FoldedDrag>,
    pub(super) resize_drag: Option<&'a ResizeDrag>,
    pub(super) transition_drag: Option<&'a TransitionDrag>,
    pub(super) clip_transition_drag: Option<&'a ClipTransitionDrag>,
    pub(super) focused_transition: Option<(crate::project::ItemAddress, TransitionSide)>,
    pub(super) import_preview: Option<&'a TimelineImportPreview>,
    pub(super) text_drop_preview: Option<&'a external_content::TextPreview>,
    pub(super) cut_preview: Option<&'a TimelineCut>,
    pub(super) live_recording: Option<&'a LiveRecordingDraw>,
    pub(super) live_video_recording: Option<LiveVideoRecordingDraw>,
    pub(super) view: TimelineViewState,
    pub(super) virtual_tracks: &'a [(TrackKind, usize)],
    pub(super) width: f64,
    pub(super) height: f64,
    pub(super) timeline_width: f64,
    pub(super) frame_step_seconds: f64,
    pub(super) animation_seconds: f64,
    pub(super) waveform_chunks_per_second: u32,
    pub(super) accent_color: Color,
    pub(super) overscroll: Option<(TimelineOverscrollEdge, f64)>,
    pub(super) horizontal_scrollbar: Option<shrimply_skia_adw_ui::Scrollbar>,
    pub(super) vertical_scrollbar: Option<shrimply_skia_adw_ui::Scrollbar>,
    pub(super) software_cursor: Option<&'a TimelineSoftwareCursor>,
}

pub(super) fn draw_timeline(input: TimelineInput<'_, '_>) {
    let TimelineInput {
        painter,
        project,
...
fn draw_timeline_overscroll(
    ...
    shrimply_skia_adw_ui::draw_overshoot(
        painter.canvas(),
...
fn draw_vertical_slider(
    painter: &TimelinePainter,
    scrollbar: Option<shrimply_skia_adw_ui::Scrollbar>,
) {
    let Some(scrollbar) = scrollbar else { return };
    shrimply_skia_adw_ui::slider::draw(painter.canvas(), scrollbar);
}
